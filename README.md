...
> "Clone https://github.com/Kaushal-Panchal/claude-memory-compiler into this project. Set up the Claude Code hooks so my conversations automatically get captured into `brain-dump/`, compiled into a knowledge base, and injected back into future sessions. Read the AGENTS.md for the full technical reference on how everything works."
...
From there, your conversations start accumulating. After 6 PM local time, the next session flush automatically triggers compilation of that day's capture into knowledge articles. You can also run `uv run python scripts/compile.py` manually at any time.

## How It Works

```
Conversation -> SessionEnd/PreCompact hooks -> flush.py extracts knowledge
    -> brain-dump/YYYY-MM-DD.md -> compile.py -> knowledge/concepts/, connections/, qa/
...
- **compile.py** turns brain-dump sources into organized concept articles with cross-references (triggered automatically or run manually)
...
```bash
uv run python scripts/compile.py                    # compile new/changed brain-dump files
